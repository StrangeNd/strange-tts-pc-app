...
function getSpeechSynthesis() {
  if (window.__STTS_FORCE_NO_SPEECH_SYNTHESIS__) return null;
  return window.speechSynthesis || null;
}

function loadVoices() {
  const synth = getSpeechSynthesis();
  state.voices = window.__STTS_FORCE_NO_VOICES__ ? [] : (synth ? synth.getVoices() : []);
  return state.voices;
}
...
function renderTtsPreviewWorkspace() {
  ...
  workspaceContent.innerHTML = `
    <form id="ttsPreviewForm" class="workspace-content tts-workspace">
      <div class="panel-header">
        <h2>Tạo giọng đọc</h2>
        <p>Nhập nội dung, chọn voice có sẵn trên máy và nghe thử ngay. Tính năng này chạy local trong trình duyệt, không gửi nội dung ra server.</p>
      </div>
      <label>
        Nội dung cần đọc
        <textarea class="textarea-input tts-textarea" name="ttsText" required maxlength="1200">Xin chào, đây là bản xem trước giọng đọc từ Strange TTS Solution.</textarea>
      </label>
      <div class="tts-helper-row">
        <span id="ttsCounter">0 / 1200 ký tự</span>
        <span>Nội dung chỉ được phát thử trên máy này.</span>
      </div>
      <div class="tts-control-grid">
        <label>
          Voice
          <select class="shop-select light-select" name="voiceIndex" ${voices.length ? '' : 'disabled'}>
            ${voiceOptions}
          </select>
        </label>
        <label>
          Tốc độ
          <input name="rate" type="range" min="0.6" max="1.4" step="0.1" value="1">
        </label>
        <label>
          Cao độ
          <input name="pitch" type="range" min="0.7" max="1.3" step="0.1" value="1">
        </label>
      </div>
      <div class="actions">
  ...
  for (const control of form.querySelectorAll('input[type="range"]')) {
    control.addEventListener('input', () => {
      if (state.tts.isPlaying) setStatus('Tốc độ/cao độ mới sẽ áp dụng ở lần nghe thử tiếp theo.', 'queued');
    });
  }
  form.addEventListener('submit', event => {
    ...
    if (!activeSynth) {
      setStatus('Không thể preview vì trình duyệt không hỗ trợ speechSynthesis.', 'error');
      return;
    }
    const data = new FormData(form);
    const text = String(data.get('ttsText') || '').trim();
    if (!text) {
      setStatus('Hãy nhập nội dung cần đọc.', 'error');
      return;
    }
    if (text.length < 2) {
      setStatus('Nội dung quá ngắn. Hãy nhập ít nhất 2 ký tự để nghe rõ hơn.', 'error');
      return;
    }
    if (text.length > 1200) {
      setStatus('Nội dung quá dài. Giới hạn hiện tại là 1200 ký tự.', 'error');
      return;
    }
    const playbackId = state.tts.playbackId + 1;
    state.tts.playbackId = playbackId;
    submitButton.disabled = true;
    window.setTimeout(() => { submitButton.disabled = false; }, 350);
    activeSynth.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    const selectedVoice = state.voices[Number(data.get('voiceIndex'))];
    if (selectedVoice) utterance.voice = selectedVoice;
    utterance.rate = Number(data.get('rate') || 1);
    utterance.pitch = Number(data.get('pitch') || 1);
    utterance.onstart = () => {
      if (state.tts.playbackId !== playbackId) return;
      state.tts.isPlaying = true;
      state.tts.lastText = text;
      setStatus('Đang phát giọng đọc thử...', 'playing');
    };
    utterance.onend = () => {
      if (state.tts.playbackId !== playbackId) return;
      state.tts.isPlaying = false;
      setStatus('Đã phát xong bản preview.', 'done');
    };
    utterance.onerror = () => {
      if (state.tts.playbackId !== playbackId) return;
      state.tts.isPlaying = false;
  ...
  bindClick('#ttsStopButton', () => {
    const activeSynth = getSpeechSynthesis();
    if (!activeSynth) {
      setStatus('Không có trình đọc giọng để dừng.', 'error');
      return;
    }
    if (!state.tts.isPlaying && !activeSynth.speaking && !activeSynth.pending) {
      setStatus('Chưa có preview nào đang phát.', 'ready');
      return;
    }
    activeSynth.cancel();
    state.tts.playbackId += 1;
    state.tts.isPlaying = false;
    setStatus('Đã dừng giọng đọc.', 'stopped');
  });
}
...
if (getSpeechSynthesis()) {
  window.speechSynthesis.onvoiceschanged = () => {
    loadVoices();
    if (document.querySelector('#ttsPreviewForm')) renderTtsPreviewWorkspace();
  };
}
...
bindClick('#btnTtsPreview', renderTtsPreviewWorkspace);
